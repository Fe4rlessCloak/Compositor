...
struct LayerMaskMenu: View {
    ...
    var body: some View {
        Button { if session.prepareForOutsideDocumentAction() { session.addMask() } } label: { Image(systemName: "rectangle.inset.filled").footerHitArea() }
            ...
            .disabled(!session.canRequestMaskEdit || session.activeLayer?.mask != nil)
    }
}
